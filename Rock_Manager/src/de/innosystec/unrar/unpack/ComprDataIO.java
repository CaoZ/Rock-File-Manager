...
public class ComprDataIO {
	...
	private ExtractIndicator extractIndicator;
	...
	public void init(FileHeader hd) throws IOException {
		...
		inputStream = new ReadOnlyAccessInputStream(archive.getRof(), startPos, startPos
				+ unpPackedSize);
		subHead = hd;
		curUnpRead = 0;
		curPackWrite = 0;
	...
	public int unpRead(byte[] addr, int offset, int count) throws IOException, RarException {
		...
		while (count > 0) {
			...
			if(extractIndicator!=null){
				extractIndicator.increaseRead(retCode);
			}
...
//			archive.bytesReadRead(retCode);
	...
	public void setExtractIndicator(ExtractIndicator indicator) {
