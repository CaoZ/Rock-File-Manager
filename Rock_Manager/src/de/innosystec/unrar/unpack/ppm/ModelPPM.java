...
public class ModelPPM {
	...
	private static int InitBinEsc[] = { 0x3CDD, 0x1F3F, 0x59BF, 0x48F3, 0x64A1, 0x5ABC, 0x6632,
			0x6051 };
	...
	public boolean decodeInit(Unpack unpackRead, int escChar/* ref */) throws IOException,
			RarException {
...
// 注释掉此句后不会出现out of memory错误了，但同时会造成其他错误。如Unpack::readTables()会失败。
// CZTODO: 目前忽略掉这个错误，可以完成对正确文件的正确解压，但若文件有crc错误不能检测出来。
// 连续从大压缩文件中解压时才会出现这个错误，如"tz7-029.rar"的020.jpg, 021.jpg.
// 解压小文件或单独解压某个文件没问题。
// 有时间后参考muCommander中rar文件的解压, 主要是SubAllocator.java
...
//			subAlloc.startSubAllocator(MaxMB + 1);
	...
	public int decodeChar() throws IOException, RarException {
		...
		if ((orderFall == 0) && foundState.getSuccessor() > subAlloc.getPText()) {
			...
			updateModel();
			// this.foundState.setAddress(foundState.getAddress());//TODO just 4
			// debugging
			if (escCount == 0) {
				clearMask();
			}
	...
	private int /* ppmcontext ptr */createSuccessors(boolean Skip, State p1 /*
		...
		upState.setSuccessor(upBranch.getAddress() + 1); // TODO check if +1
															// necessary
		if (pc.getNumStats() != 1) {
	...
	private void updateModel() {
		...
		for (pc.setAddress(maxContext.getAddress()); pc.getAddress() != minContext.getAddress(); pc
				.setAddress(pc.getSuffix())) {
			if ((ns1 = pc.getNumStats()) != 1) {
				if ((ns1 & 1) == 0) {
					// System.out.println(ns1);
					pc.getFreqData().setStats(
							subAlloc.expandUnits(pc.getFreqData().getStats(), ns1 >>> 1));
				...
				int sum = ((2 * ns1 < ns) ? 1 : 0)
						+ 2
						* (((4 * ns1 <= ns) ? 1 : 0) & ((pc.getFreqData().getSummFreq() <= 8 * ns1) ? 1
								: 0));
