...
public class RarVM extends BitInput {
	...
	public void setLowEndianValue(Vector mem, int offset, int value) {
		...
		for (int i = 0; i <= 3; i++) {
			mem.setElementAt(new Byte(bytes[i]), offset + i);
		}
	...
	public void execute(VMPreparedProgram prg) {
		...
		if (globalSize != 0) {
			...
			{
				mem[VM_GLOBALMEMADDR + i] = ((Byte) prg.getGlobalData().elementAt(i)).byteValue();
			}
		...
		if (staticSize != 0) {
			...
			{
				mem[VM_GLOBALMEMADDR + (int) globalSize + i] = ((Byte) prg.getStaticData()
		...
		if (!ExecuteCode(preparedCode, prg.getCmdCount())) {
			...
			((VMPreparedCommand) preparedCode.firstElement()).setOpCode(VMCommands.VM_RET);
		}
		...
		int dataSize = Math.min(getValue(false, mem, VM_GLOBALMEMADDR + 0x30), VM_GLOBALMEMSIZE
				- VM_FIXEDGLOBALSIZE);
		if (dataSize != 0) {
			...
			for (int i = 0; i < dataSize + VM_FIXEDGLOBALSIZE; i++) // memcpy(&Prg->GlobalData[0],&Mem[VM_GLOBALMEMADDR],DataSize+VM_FIXEDGLOBALSIZE);
			{
				...
				prg.getGlobalData().setElementAt(new Byte(mem[VM_GLOBALMEMADDR + i]), i);
			}
		}
	}
		...
		while (true) {
			...
			switch (cmd.getOpCode()) {
				...
				case VMCommands.VM_ADD: {
					int value1 = getValue(cmd.isByteMode(), mem, op1);
					int result = (int) ((((long) value1 + (long) getValue(cmd.isByteMode(), mem,
							op2))) & 0xffffffff);
					if (cmd.isByteMode()) {
						result &= 0xff;
						flags = (result < value1) ? 1 : 0 | (result == 0 ? VMFlags.VM_FZ
								: ((result & 0x80) != 0) ? VMFlags.VM_FS : 0);
						// Flags=(Result<Value1)|(Result==0 ?
						// VM_FZ:((Result&0x80) ?
						// VM_FS:0));
					} else
						flags = (result < value1) ? 1 : 0 | (result == 0 ? VMFlags.VM_FZ
								: (result & VMFlags.VM_FS));
					setValue(cmd.isByteMode(), mem, op1, result);
				}
					break;

				case VMCommands.VM_ADDB:
					setValue(true, mem, op1, (int) ((long) getValue(true, mem, op1) & 0xFFffFFff
							+ (long) getValue(true, mem, op2) & 0xFFffFFff));
					break;
				case VMCommands.VM_ADDD:
					setValue(false, mem, op1, (int) ((long) getValue(false, mem, op1) & 0xFFffFFff
				...
				case VMCommands.VM_SUB: {
					...
					flags = (result == 0) ? VMFlags.VM_FZ : (result > value1) ? 1
							: 0 | (result & VMFlags.VM_FS);
				...
				case VMCommands.VM_INC: {
					...
					setValue(cmd.isByteMode(), mem, op1, result);
					flags = result == 0 ? VMFlags.VM_FZ : result & VMFlags.VM_FS;
				}
					break;
				...
				case VMCommands.VM_SHL: {
					...
					flags = (result == 0 ? VMFlags.VM_FZ : (result & VMFlags.VM_FS))
							| (((value1 << (value2 - 1)) & 0x80000000) != 0 ? VMFlags.VM_FC : 0);
					setValue(cmd.isByteMode(), mem, op1, result);
				}
					break;
				...
				case VMCommands.VM_NEG: {
					int result = -getValue(cmd.isByteMode(), mem, op1);
					flags = result == 0 ? VMFlags.VM_FZ : VMFlags.VM_FC | (result & VMFlags.VM_FS);
					setValue(cmd.isByteMode(), mem, op1, result);
				}
					break;
				...
				case VMCommands.VM_ADC: {
					int value1 = getValue(cmd.isByteMode(), mem, op1);
					int FC = (flags & VMFlags.VM_FC);
					int result = (int) ((long) value1 & 0xFFffFFff
							+ (long) getValue(cmd.isByteMode(), mem, op2) & 0xFFffFFff + (long) FC & 0xFFffFFff);
					if (cmd.isByteMode()) {
						result &= 0xff;
					}

					flags = (result < value1 || result == value1 && FC != 0) ? 1
							: 0 | (result == 0 ? VMFlags.VM_FZ : (result & VMFlags.VM_FS));
					setValue(cmd.isByteMode(), mem, op1, result);
				}
					break;
				...
				case VMCommands.VM_STANDARD:
					ExecuteStandardFilter(cmd.getOp1().getData());
					break;
	...
	public void prepare(byte[] code, int codeSize, VMPreparedProgram prg) {
		...
		if (xorSum == code[0]) {
			...
			int filterType = IsStandardFilter(code, codeSize);
			...
			if (filterType != VMStandardFilters.VMSF_NONE) {
				...
				curCmd.getOp1().setData(filterType);
			...
			if ((dataFlag & 0x8000) != 0) {
				...
				for (int i = 0; inAddr < codeSize && i < dataSize; i++) {
					prg.getStaticData().addElement(new Byte((byte) (fgetbits() >> 8)));
					faddbits(8);
				}
			}
			...
			while (inAddr < codeSize) {
				...
				if ((VMCmdFlags.VM_CmdFlags[curCmd.getOpCode()] & VMCmdFlags.VMCF_BYTEMODE) != 0) {
				...
				int opNum = (VMCmdFlags.VM_CmdFlags[curCmd.getOpCode()] & VMCmdFlags.VMCF_OPMASK);
				...
				if (opNum > 0) {
					...
					if (opNum == 2)
						...
						if (curCmd.getOp1().getType() == VMOpType.VM_OPINT
								&& (VMCmdFlags.VM_CmdFlags[curCmd.getOpCode()] & (VMCmdFlags.VMCF_JUMP | VMCmdFlags.VMCF_PROC)) != 0) {
	...
	private void optimize(VMPreparedProgram prg) {
		...
		for (int j = 0; j < commands.size(); j++) {
			...
			if ((VMCmdFlags.VM_CmdFlags[cmd.getOpCode()] & VMCmdFlags.VMCF_CHFLAGS) == 0) {
				continue;
			}
			boolean flagsRequired = false;

			for (int i = commands.indexOf(cmd) + 1; i < commands.size(); i++) {
				int flags = VMCmdFlags.VM_CmdFlags[((VMPreparedCommand) commands.elementAt(i))
						.getOpCode()];
	...
	private int IsStandardFilter(byte[] code, int codeSize) {
	...
	private void ExecuteStandardFilter(int filterType) {
		switch (filterType) {
						...
						if (cmdMask != 0)
							for (int i = 0; i <= 2; i++)
								if ((cmdMask & (1 << i)) != 0) {
									...
									if (opType == 5) {
										int offset = filterItanium_GetBits(curPos, startPos + 13,
												20);
										filterItanium_SetBits(curPos,
			...
			case VMStandardFilters.VMSF_DELTA: {
				...
				// bytes from same channels are grouped to continual data
				// blocks,
