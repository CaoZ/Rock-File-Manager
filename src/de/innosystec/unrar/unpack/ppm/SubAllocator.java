...
public class SubAllocator {
	...
	private void glueFreeBlocks() {
		...
		for (p.setAddress(s0.getNext()); p.getAddress() != s0.getAddress(); p.setAddress(p
				.getNext())) {
		...
		while (p.getAddress() != s0.getAddress()) {
			for (p.remove(), sz = p.getNU(); sz > 128; sz -= 128, p.setAddress(MBPtr(
					p.getAddress(), 128))) {
	...
	public void initSubAllocator() {
		int i, k;
		// Arrays.fill(heap, freeListPos, freeListPos + sizeOfFreeList(), (byte)
		// 0);
		Arrays.fill(heap, (byte) 0, freeListPos, sizeOfFreeList());
